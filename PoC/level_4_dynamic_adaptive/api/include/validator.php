 ...
 */
	 ...
	 *   2) Verificar que existan límite máximo de encabezados válidos definidos.
	 *   3) Detectar encabezados no registrados.
	...
	//  Validar encabezados señuelo debe ser un arreglo
	if (!is_array(RHC_HEADERS['decoys'])) {
		$mensaje = "RHC — La lista de encabezados señuelo (decoys) no es válida.";
		// Regresa un Error
		Response::error($mensaje, 500); // Internal Server Error
		exit();
	}
	//  Validar encabezados señuelo pero está vacía
	if (empty(RHC_HEADERS['decoys'])) {
		$mensaje = "RHC — No hay encabezados señuelos definidos en api/config/rhc_config.php.";
		// Regresa un Error
		Response::error($mensaje, 500); // Internal Server Error
		exit();
	}
	...
	// ============================================================================
	// 2️⃣ Validar límite máximo de encabezados válidos
	// ============================================================================
	if (!defined('RHC_MAX_VALID_HEADERS') || !is_int(RHC_MAX_VALID_HEADERS) || RHC_MAX_VALID_HEADERS < 1) {
		$mensaje = "RHC — La constante RHC_MAX_VALID_HEADERS no está definida correctamente en api/config/rhc_config.php.";
		// Regresa un Error
		Response::error($mensaje, 500); // Internal Server Error
		exit();
	}
	...
	// Obtener y normalizar encabezados decoys esperados
	// Se convierten a formato interno utilizado por PHP (HTTP_X_...)
	$decoysEsperados = [];
	foreach (RHC_HEADERS['decoys'] as $header) {
		$header = str_replace('-', '_', strtoupper($header));
		if (strpos($header, 'HTTP_') !== 0) {
			$header = 'HTTP_' . $header;
		}
		$decoysEsperados[] = $header;
	}
	...
	$recibidosDecoys  = array_intersect(array_keys($headersRecibidos), $decoysEsperados);

	// ==========================================================
	// 3️⃣ Detectar encabezados no registrados
	// ==========================================================
	$todosEsperados = array_merge($validosEsperados, $decoysEsperados);

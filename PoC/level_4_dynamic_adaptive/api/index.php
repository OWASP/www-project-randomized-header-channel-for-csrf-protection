 ...
 */
	...
	// Incluir RHC (Randomized Header Channel) Nivel 4 — Adaptativo Dinámico
	require_once __DIR__ . '/middleware/rhc_dynamic_adaptive.php';

	// Importar la clase principal del middleware RHC
	use Middleware\RHC\RHC_DynamicAdaptive;

	// Crear una instancia del middleware RHC Nivel 4
	$rhc = new RHC_DynamicAdaptive(
		RHC_HEADERS['validos'], // Lista de headers válidos.
		RHC_HEADERS['decoys'], // Lista de headers decoy (falsos).
		RHC_MAX_VALID_HEADERS // Número máximo de headers válidos permitidos por petición.
	);

	// Ejecutar la validación de headers en la petición actual.
	$selectedHeaders = $rhc->validate();
